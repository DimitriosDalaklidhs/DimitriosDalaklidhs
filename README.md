...
Final year Informatics student at the University of Western Macedonia (THE WUR CS: 601 – 800, 2025), graduating July 2026. I work primarily in C and Java, with a focus on systems programming, network applications, and backend development. Comfortable at the low level: sockets, process management, memory, file I/O. Experienced building production grade backend services with REST APIs, JWT auth, role-based access control, and relational databases. Open-source contributor to ion fusion (Amazon). Open to roles in systems, backend, or software engineering.
&nbsp;
...
Secure REST API for managing hospital patients, hospitalizations, and medical tests. Features stateless JWT authentication, role based access control (DOCTOR / CLERK / ADMIN) enforced at the HTTP method level, and atomic patient registration via Spring `@Transactional`. Includes a full Luhn algorithm implementation for validating Greek ΑΜΚΑ numbers with birth date extraction. Unit tested with JUnit 5 and Mockito across auth, JWT, and user detail layers. Deployed on Railway!
